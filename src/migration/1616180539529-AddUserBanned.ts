...
export class AddUserBanned1616180539529 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "temporary_user" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "name" varchar NOT NULL, "passwordHash" varchar NOT NULL, "loginToken" varchar, "balance" integer NOT NULL DEFAULT (0), "totalIn" integer NOT NULL DEFAULT (0), "totalOut" integer NOT NULL DEFAULT (0), "joined" datetime NOT NULL DEFAULT (DATETIME('now')), "banned" boolean NOT NULL DEFAULT (0), CONSTRAINT "UQ_065d4d8f3b5adb4a08841eae3c8" UNIQUE ("name"))`);
        await queryRunner.query(`INSERT INTO "temporary_user"("id", "name", "passwordHash", "loginToken", "balance", "totalIn", "totalOut", "joined") SELECT "id", "name", "passwordHash", "loginToken", "balance", "totalIn", "totalOut", "joined" FROM "user"`);
